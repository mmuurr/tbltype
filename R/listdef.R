...
listdef.el <- function(cast = ident,
  ...
  list(
    ...
    nullas = nullas,
    castnull = castnull
  ) %>% prepend_class("tbltype.listdef.el")
}
...
listdef <- function(eldefs = list(), klass = NULL) {
  ...
  purrr::walk(eldefs, function(eldef) {
    checkmate::assert(
      checkmate::check_class(eldef, "tbltype.listdef.el"),
      checkmate::check_class(eldef, "tbltype.listdef"),
      checkmate::check_class(eldef, "tbltype.tbldef"),
      combine = "or"
    )
  })
  ...
  cast <- function(x = list(), .keep_all = FALSE, ...) {
    ...
    out <- purrr::map(elnames, function(elname) {
      ...
      outel <-
        if (!exists(elname, x)) {
          if (inherits(eldefs[[elname]], c("tbltype.listdef", "tbltype.tbldef"))) {
            NULL
          } else {  ## must be tbltype.listdef.el
  ...
  return(list(cast = cast, klass = klass, ptype = cast(list())) %>% prepend_class("tbltype.listdef"))
}  
